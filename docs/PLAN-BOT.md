...
5. Ответить Twitch `204` **независимо от результата отправки**: событие уже
   принято (доставка «хотя бы раз»), а не-2xx заставит Твич повторять запрос и в
   итоге отозвать подписку. Ошибку постинга сообщаем админу в личку.
6. Описание сбоя строит `src/errors.ts`: `describeTelegramFailure()` берёт
   `code`/`description`/`parameters` из `TelegramError` и печатает их как есть —
   текст ошибки не разбираем, его формат от нас не зависит. Строка `💡` берётся
   только из структурированных `parameters` (`retry_after`,
   `migrate_to_chat_id`); что делать с остальными частыми ошибками — таблица в
   README.
...
1. **`EVENTSUB_SECRET` неизменяем**: после создания подписки менять нельзя — все
   ...
   подписку.
2. **Дубли** `stream.online` — Твич доставляет «хотя бы раз» и повторяет запрос,
   если не получил 2xx. Поэтому `api/twitch.ts` отвечает `204` **всегда**, в том
   числе когда пост в Telegram не удался: ошибка уходит уведомлением в личку, а
   не ретраем от Твича. Дубль возможен только если Твич повторит доставку из-за
   таймаута функции; полная защита — дедуп по `twitch-eventsub-message-id` через
